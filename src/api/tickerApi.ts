...
// Гибридный API: пробуем сервер, если не получается - используем localStorage
export const fetchTickerSettings = async () => {
  console.log('🔄 Fetching ticker settings...');
  
  // Сначала пробуем получить с сервера (используя защищенный маршрут админки)
  try {
    ...
    if (res.ok) {
      ...
      if (Array.isArray(data)) {
        // API возвращает массив настроек, находим ticker
        const tickerSetting = data.find((item: any) => item.name === 'ticker');
        if (tickerSetting) {
          const settings = {
            ...tickerSetting.value,
            is_enabled: tickerSetting.enabled, // enabled на верхнем уровне!
            source: 'api'
          };
          
          // Сохраняем полученные данные в localStorage как бэкап
          localStorage.setItem(STORAGE_KEY, JSON.stringify(settings));
          return settings;
        }
      } else if (data.ticker) {
        // API возвращает объект с ключом ticker
        const settings = {
          ...data.ticker.value,
          is_enabled: data.ticker.enabled, // enabled на верхнем уровне!
          source: 'api'
        };
